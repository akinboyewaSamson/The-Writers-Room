...
export default function AuthorPostEditor() {
  ...
  const submit = async (payload) => { setSubmitting(true); setError(''); try { if (editing) await updatePost(id, payload); else await createPost(payload); navigate('/author/posts'); } catch (err) { setError(getApiError(err)); } finally { setSubmitting(false); } };
